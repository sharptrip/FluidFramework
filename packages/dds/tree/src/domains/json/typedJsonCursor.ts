...
export class TypedJsonCursor<T> extends JsonCursor<T> {
    ...
    constructor(
        ...
        parentType: NamedTreeSchema,
        parentKey: FieldKey,
        root: Jsonable<T>,
    ) {
        super(root);
        this.currentType = this.findTypeForJsonable(parentType, parentKey, root);
    }
    ...
    public down(key: FieldKey, index: number): SynchronousNavigationResult {
        ...
        if (result === TreeNavigationResult.Ok) {
            ...
            this.currentType = this.findTypeForJsonable(this.currentType, key, this.value as Jsonable<T>);
        }
    ...
    private findTypeForJsonable(parentType: NamedTreeSchema, key: FieldKey, value: Jsonable<T>): NamedTreeSchema {
