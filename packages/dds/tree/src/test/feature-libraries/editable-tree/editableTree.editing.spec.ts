...
// eslint-disable-next-line import/no-internal-modules
import { copyAsDetachedField } from "../../../feature-libraries/editable-tree/editableTree";
import {
    complexPhoneSchema,
    ComplexPhoneType,
    float32Schema,
...
import { expectFieldEquals } from "./utils";
...
describe.only("editing with editable-tree", () => {
    it("detached fields", async () => {
        const root2 = brand<FieldKey>("root2");
        const [provider, trees] = await createSharedTrees(fullSchemaData, [personData], 2);
        const rootSchema = trees[0].context.root.fieldSchema;
        const second = copyAsDetachedField(trees[0].context.root);
        const root0_2 = trees[0].context.getDetachedField(root2, rootSchema, false);
        expectFieldEquals(trees[0].forest.schema, second, [personData]);
        expectFieldEquals(trees[0].forest.schema, root0_2, [personData]);
        const secondPerson = second[0] as PersonType;
        secondPerson.name = "Joe";
        assert.notEqual(secondPerson.name, (trees[0].root as PersonType).name);
        await provider.ensureSynchronized();
        const root1_2 = trees[1].context.getDetachedField(root2, rootSchema, true) as PersonType;
        assert.equal(secondPerson.name, root1_2.name);
        trees[0].context.free();
        trees[1].context.free();
    });

    it("create field using `insertNodes`", async () => {
        ...
        const newZipCursor = singleTextCursor({ value: "99038", type: stringSchema.name });
        zipField.insertNodes(0, newZipCursor);
        assert.throws(
            () => zipField.insertNodes(1, newZipCursor),
            (e) =>
                validateAssertionError(e, "A non-sequence field cannot have more than one node."),
    ...
    it("create field using `createFieldSymbol`", async () => {
        const zip: LocalFieldKey = brand("zip");
        const [provider, trees] = await createSharedTrees(fullSchemaData, [personData], 2);
        const person = trees[0].root as PersonType;
        assert.equal(zip in person.address, false);
        ...
        const newZipCursor = singleTextCursor({ value: "123.456", type: float32Schema.name });
        person.address[createFieldSymbol](zip, newZipCursor);
        assert.equal(zip in person.address, true);
    ...
    it("update property", async () => {
        ...
        const person2 = trees[1].root as PersonType;
        person1.address[createFieldSymbol](
            brand("zip"),
    ...
    it("delete property", async () => {
        ...
        delete person.address.phones;
        assert.equal(address.phones, undefined);
        ...
        person.address[createFieldSymbol]("phones" as FieldKey, phonesCursor);
