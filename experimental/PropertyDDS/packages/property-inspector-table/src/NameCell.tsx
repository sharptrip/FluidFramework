...
const editableTreeDeletionHandler = async (rowData: IEditableTreeRow) => {
	assert(isEditableTree(rowData.data), "requires data as node");
	...
	if (
		...
		rowData.data[parentField].parent.fieldSchema.kind.identifier ===
		...
		rowData.parent.removeNodes(rowData.data[parentField].index, 1);
	} else {
		rowData.parent.remove();
	}
...
function getDeleteHandler({ rowData, readOnly }: Partial<INameCellProps & ICellProps>) {
	...
	if (isEditableTreeRow(rowData)) {
		return { handler: async () => editableTreeDeletionHandler(rowData) };
	}
	if (
...
const NameCell: React.FunctionComponent<
	WithStyles<typeof styles> & INameCellProps & ICellProps
> = ({
	rowData,
	iconRenderer,
	classes,
	...
	return (
		<div className={classNames(classes.rowContainer, className)}>
			<div className={classes.textAndIconContainer}>
				...
				<OverflowableCell cellContent={rowData.name} />
			</div>
			<div className={classNames(classes.menuGravity)} ref={ref}>
				<ItemMenu
					...
					options={{
						copy: isEditableTreeRow(rowData)
							? undefined
