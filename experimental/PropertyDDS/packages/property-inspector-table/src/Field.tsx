...
import { EditableTreeRow, IEditableValueCellProps, IInspectorRow, isEditableTreeRow } from "./InspectorTableTypes";
...
function onInlineEditEnd(val: string | number | boolean, props: IEditableValueCellProps) {
    const rowData: IInspectorRow | EditableTreeRow = props.rowData;
    ...
    if (isEditableTreeRow(rowData)) {
        const parent = rowData.parent ?? fail("root may not be edited");
        try {
            parent[rowData.nodeIndex] = val;
        } catch (e) {
            console.error(e);
        }
        return;
