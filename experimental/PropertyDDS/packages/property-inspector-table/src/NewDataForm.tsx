...
const addCorrespondingSvgIcon = (
	propOptions: INewDataFormOptions[],
): DecoratedSelectOptionsType => {
...
const setContext: IDecoratedSelectOptionType = {
	value: "set",
	label: "Set",
...
export const NewDataForm: React.FunctionComponent<INewDataFormProps> = (props) => {
	...
	if (parentContext !== "single" || isNewNode) {
	...
	const nameInput = (height: number, width: number) => {
		...
		const selectedTypeOrCollectionLabel =
			selectedContainerOption.value === "single"
			...
			{({ width, height }) => (
				<div
					...
					className={classNames(
						classes.container,
						classes.selectDropdownElevated,
						classes.borderRadiusCommon,
					)}
				>
