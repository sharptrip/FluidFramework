...
import { getEditableTree, ISharedTree, proxyTargetSymbol, SharedTreeFactory } from "@fluid-internal/tree";
...
// import { SharedTreeContainerRuntimeFactory as ContainerFactory } from "./containerCode";
...
async function start(): Promise<void> {
    ...
    // const sharedTree: ISharedTree = await getDefaultObjectFromContainer<ISharedTree>(container);

    const sharedTree = container.initialObjects.sharedTree as ISharedTree;

    await buildProxy(sharedTree, !documentId);

    const [context, editableTree] = getEditableTree(sharedTree);

    const rootTarget = editableTree?.[proxyTargetSymbol];

    renderApp({ context, editableTree }, document.getElementById("root")!);
}
