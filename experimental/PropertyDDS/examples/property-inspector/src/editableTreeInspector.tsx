...
import { getPerson } from "./demoData";
import { isEmptyRoot, isSequenceField, getFieldSource, getDefaultValueIfPrimitive } from "./utils";
...
const tableProps: Partial<IInspectorTableProps> = {
	...
	dataCreationHandler: async (
		...
		try {
			if (isEditableTree(parent)) {
				...
				const newField = parent[getField](fieldKey);
				const newNodeData =
					context === "single"
						? {
								[typeNameSymbol]: typeName,
								[valueSymbol]: getDefaultValueIfPrimitive(
									newField.context.schema,
									typeName,
								),
						  }
						: {
								[typeNameSymbol]: brand<TreeSchemaIdentifier>(
									`${context}<${typeid}>`,
								),
						  };
				newField.setContent(isSequenceField(newField) ? [newNodeData] : newNodeData);
			} else if (isEditableField(parent)) {
				const newNodeData = {
					[typeNameSymbol]: typeName,
					[valueSymbol]: getDefaultValueIfPrimitive(parent.context.schema, typeName),
				};
				if (isSequenceField(parent)) {
					parent.insertNodes(Number(name), [newNodeData]);
				} else {
					parent.setContent(newNodeData);
				}
			} else {
				unreachableCase(parent);
			}
		} catch (e) {
			console.error(e);
		}
	...
	expandAll: (data: ISharedTree): IExpandedMap => {
		...
		return expandField(root, {}, { pathPrefix: "" });
	},
};
...
function expandNode(
	...
	return node[forEachField](expandField, expanded, { pathPrefix: id });
}
...
function addNewDataLine(
	...
	const typeid =
		isEditableField(parent) && isSequenceField(parent) && parent.fieldSchema.types?.size === 1
			? [...parent.fieldSchema.types][0]
			: "";
	rows.push({
		id: `${pathPrefix}/Add`,
		isNewDataRow: true,
...
function nodeToTableRow(
	...
	const id = getRowId(parent, pathPrefix, index);
	const keyAsString = parent.parent ? parent.fieldKey : "Person";
	...
	if (!isPrimitive(nodeType)) {
		newRow.children = data[forEachField](fieldToTableRow, [], { pathPrefix: id });
...
function fieldToTableRow(
	...
	if (isSequence && field.parent && getPrimaryField(field.parent[typeSymbol]) === undefined) {
		...
		const id = getRowId(field, pathPrefix);
		const children = field.forEachNode(nodeToTableRow, [], { pathPrefix: id });
		addNewDataLine(children, field, id);
		const newRow: IEditableTreeRow = {
			id,
			name: field.fieldKey,
			context: "single",
		...
		if (isEmptyRoot(field) || isSequence) {
			addNewDataLine(rows, field, pathPrefix);
		}
	}
	return rows;
}
...
const editableTreeTableProps: Partial<IInspectorTableProps> = {
	...
	toTableRows: ({ data }: { data: ISharedTree }): IEditableTreeRow[] => {
		return fieldToTableRow(data.context.root, [], { pathPrefix: "" });
	},
};
							...
							<Toolbar>
								<div className={classes.buttonContainer}>
									<Button
										...
										onClick={() =>
											sharedTree.setContent(
												isSequenceField(sharedTree.context.root)
													? [getPerson()]
													: getPerson(),
											)
										}
...
export function renderApp(sharedTree: ISharedTree) {
	...
	if (!context.fieldSource) {
		context.fieldSource = getFieldSource(context);
	}
	const render = () => {
		context.clear();
