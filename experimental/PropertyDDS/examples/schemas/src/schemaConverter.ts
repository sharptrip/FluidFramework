...
export const fieldKinds = {
    ...
    value: brand<FieldKindIdentifier>("Value"),
    sequence: brand<FieldKindIdentifier>("Sequence"),
};

export function convertPSetSchema(rootTypes: TreeTypeSet, fieldKind: FieldKindIdentifier): SchemaData {
    const treeSchema = new Map();

    if (rootTypes === undefined) {
        throw new Error(`Root types are required`);
    }
    ...
    const rootFieldSchema: FieldSchema = {
        kind: fieldKind,
        types: rootTypes,
    };
